...
module.exports.put_genre = async (req, res) => {
    if (isLogin(req)) {
        const { old_genre, new_genre } = req.body
        const research = await Film.updateMany({ genre: old_genre }, { $set: { genre: new_genre } })
        if(research) {
            return res.status(200).json({ msg: "Update successful!" })
        }
        return res.status(400).json({ err: "Update failure!" })
    }
    return res.redirect('/login')
}
...
module.exports.update_user = async (req, res) => {
    if (isLogin(req)) {
        try {
            ...
            const user = await Account.findOne({ username: update.username })
            if(user)
                return res.status(400).json({ err: "username or email existed or nothing changed!" });
            const doc = await Account.findOneAndUpdate({ id }, update)
            if(doc)
                return res.status(200).json({ msg: "Update successful!" });
            return res.status(400).json({ err: "Update failure!" });
        }
